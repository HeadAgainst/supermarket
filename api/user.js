...
export default {
	...
	getAddress(address_id) {
	      return http.get(`/addresses/${address_id}`);
	},
	//获取所有评论图⽚URL列表 ( GET /reviews_img/:review_id )
	getReviewimgURl(review_id){
		return http.get(`/reviews_img/${review_id}`);
	},
	//获取评论图片
	getgetReviewimg(review_id,code){
		return http.get(`/reviews_img/${review_id}/${code}`);
	},
	postReviewimg(review_id, data) {
	    return http.post(`/reviews_img/${review_id}`, { image: data });
	}
}